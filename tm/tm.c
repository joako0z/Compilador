...
{ printf( "%5d: ", loc) ;
  ...
  { printf("%6s%3d,", opCodeTab[iMem[loc].iop], iMem[loc].iarg1);
    ...
    { case opclRR: printf("%5d,%5d", iMem[loc].iarg2, iMem[loc].iarg3);
                   break;
      case opclRM:
      case opclRA: printf("%3d(%5d)", iMem[loc].iarg2, iMem[loc].iarg3);
                   break;
    }
    printf ("\n") ;
  }
  ...
  { sign = 1;
    ...
    { 
        ...
        else
        {
          if(decimal==1)
          {  
             numDecimal = (( ch - '0' )/decimales);
             term = term + numDecimal;
             decimales = decimales * 10.0f;
          }
          else
          {
             term = term * 10 + ( ch - '0' ) ;
...
{ char cmd;
  ...
  { case 't' :
    ...
    /***********************************/
      if ( atEOL ())  stepcnt = 1;
      else if ( getNum ())  stepcnt = fabs(num);
